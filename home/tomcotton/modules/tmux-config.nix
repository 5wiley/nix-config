  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.tmux = {
      ...
      extraConfig = lib.mkAfter ''
        ...
        # Swap split bindings (reverse defaults)
        bind '"' split-window -h  # " now splits vertically (pane to the right)
        bind '%' split-window -v  # % now splits horizontally (pane below)

        # Switch to sessions by index (prefix C-1 for first session, etc.)
        ...
        # better windown focus styling - gruvbox-material
        set-window-option -g window-style 'bg=#3c3836,dim'      # Inactive: gruvbox grey with desaturated colors
        set-window-option -g window-active-style 'bg=#282828'   # Active: gruvbox-material dark background
        set -g pane-border-style 'fg=colour237,bg=#32302f'       # Inactive border
        set -g pane-active-border-style 'fg=colour142,bg=#282828' # Active border (gruvbox green)
