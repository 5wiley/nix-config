  ...
  monitorScript = pkgs.writeShellScript "loki-host-monitor" ''
    ...
    for host in ${concatStringsSep " " cfg.expectedHosts}; do
      count=$(${pkgs.curl}/bin/curl -sG "$LOKI_URL/loki/api/v1/query" \
        --data-urlencode "query=sum(count_over_time({job=\"systemd-journal\", hostname=\"$host\"}[1h]))" \
        --data-urlencode "time=$(${pkgs.coreutils}/bin/date +%s)" \
        --max-time 10 2>/dev/null \
        | ${pkgs.jq}/bin/jq '.data.result[0].value[1] // "0" | tonumber' 2>/dev/null \
        || echo 0)
    ...
    for host in ${concatStringsSep " " cfg.expectedHosts}; do
      error_count=$(${pkgs.curl}/bin/curl -sG "$LOKI_URL/loki/api/v1/query" \
        --data-urlencode "query=sum(count_over_time({job=\"systemd-journal\", hostname=\"$host\", priority=~\"err|crit|alert|emerg\"}[1h]))" \
        --data-urlencode "time=$(${pkgs.coreutils}/bin/date +%s)" \
        --max-time 10 2>/dev/null \
        | ${pkgs.jq}/bin/jq '.data.result[0].value[1] // "0" | tonumber' 2>/dev/null \
        || echo 0)
